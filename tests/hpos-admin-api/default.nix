...
makeTest {
  ...
  testScript = ''
    start_all()
    machine.succeed(
        "hpos-config-gen-cli --email test\@holo.host --password : --seed-from /nix/store/5yd6jxl41ma3vmd5slf0p5fm10zra5nh-seed.txt > /etc/hpos-config.json"
    )
    ...
    actual_settings = machine.succeed(
        "hpos-admin-client --url=http://localhost/tests/ get-settings"
    ).strip()
    ...
    with subtest(
        ...
        expected_stats = (
            "'stats': {'traffic': {'start_date': None, 'total_zome_calls': 0, 'value': []}"
        )

        actual_hosted_happs = machine.succeed(
            "hpos-admin-client --url=http://localhost/tests/ get-hosted-happs"
        ).strip()
        ...
        assert (
            expected_publisher in actual_hosted_happs
        ), "unexpected_hosted_happs_publisher"
        ...
        assert (
            expected_number_instances in actual_hosted_happs
        ), "unexpected_hosted_happs_number_instances"
